            ...
            {reports.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border px-4 py-6 text-center">
                ...
                <p className="mt-1 text-body-sm text-muted-foreground">
                  Start a report, reproduce the bug, and TRAIL writes the issue for you.
                </p>
              </div>
            ) : (
      ...
      {view === 'setup' && (
        <div className="flex flex-col gap-4">
          <p className="text-body-sm text-muted-foreground">
            Open the page with the bug, then begin recording. TRAIL captures clicks, typed
            input, console errors, and failed requests.
          </p>
          <div className="flex items-start justify-between gap-3 rounded-xl border border-border p-3">
            <div className="flex items-start gap-2.5">
              <ShieldCheck className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" />
              <div>
                <Label className="text-[13px] font-medium">Auto-redact typed values</Label>
          ...
          <div className="flex gap-2">
            <Button variant="secondary" className="flex-1" onClick={() => setView('home')} disabled={busy}>
              Back
            </Button>
            <Button className="flex-1" id="begin" onClick={() => void start()} disabled={busy}>
              Begin Recording
            </Button>
          </div>
        </div>
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            “{title}” and its saved replay will be permanently removed. This can’t be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
