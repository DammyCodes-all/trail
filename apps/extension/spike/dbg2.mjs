...
await sleep(2000);
console.log('alert:', await popup.evaluate(() => window.__lastAlert));
console.log('page1 __trailRecorder:', await testPage.evaluate(() => window.__trailRecorder));
await testPage.evaluate(() => console.error('debug hello'));
await testPage.evaluate(() => document.getElementById('boom').click());
await testPage.evaluate(() => fetch('/missing').catch(()=>{}));
await sleep(2500);
