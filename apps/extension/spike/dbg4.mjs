...
const dumpDb = (page) => page.evaluate(async () => {
  const db = await new Promise((res, rej) => { const r = indexedDB.open('trail', 1); r.onsuccess = () => res(r.result); r.onerror = () => rej(r.error); });
  const all = await new Promise((res) => { const q = db.transaction('events').objectStore('events').getAll(); q.onsuccess = () => res(q.result); });
  return all.map((e) => ({ k: e.k, url: e.url, target: e.target, label: e.label, lv: e.lv, status: e.status, t: e.t }));
});
...
await testPage.evaluate(() => { window.__boomEl = document.getElementById('boom'); });
await sleep(2500);
console.log('DB after boom click (before nav):');
console.log(JSON.stringify(await dumpDb(popup), null, 1));
await browser.close();
srv.kill();
console.log('done');
