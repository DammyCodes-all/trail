...
const SYSTEM_PROMPT = [
  ...
  "- A flag's 'note' is the reporter's free-form account: it may mix what they",
  "  observed with what they expected in one sentence. When the chosen template",
  "  has expected-outcome and actual-outcome fields (e.g. 'Expected behavior',",
  "  'What did you expect to happen?', 'What happened instead?'), extract the",
  "  respective parts from the note and fill them verbatim; never invent",
  "  expected/actual content the note does not contain. Legacy flags may carry",
  "  the structured 'expected'/'actual' notes instead — fill those template",
  "  fields from them directly when present.",
  ...
  "- Treat observed client-side evidence (status codes, console errors, step",
  "  outcomes, reporter flags) as facts, but never assert certainty about",
  "  anything the digest does not directly show: server-side state, databases,",
  "  configuration, or third-party services. Phrase inferences with 'likely',",
  "  'appears to be', or 'may be'.",
...
const TITLE_SYSTEM_PROMPT = [
  ...
  "  reporter's phrasing.",
  "- A flag's 'note' is the reporter's free-form account: it may describe what",
  "  happened, what they expected, or both, in one sentence. Extract the",
  "  expected-outcome and actual-outcome parts from it when the template asks",
  "  (e.g. 'What did you expect to happen?' / 'What happened instead?'); keep",
  "  the reporter's wording and never invent either side when the note does",
  "  not say it.",
  ...
  "- Never attribute a cause in the title; name only what the evidence shows.",
  "- Return exactly one JSON object. No Markdown, no code fences, no commentary.",
].join("\n");
